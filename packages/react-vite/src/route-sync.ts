 ...
 * @module @micro-app-adapter/react-vite/route-sync
 *
 * @description
 * 该模块实现了主子应用之间的路由双向同步：
 * 1. 主应用路由变化时，自动同步到子应用
 * 2. 子应用路由变化时，自动同步到主应用
 * 3. 使用 React 18 原生 useSyncExternalStore Hook
 */

import React, { JSX, useCallback, useEffect, useRef, useSyncExternalStore } from 'react'
import {
  MemoryRouter,
  Routes,
  useNavigate,
...
/** React Hook：同步微前端路由状态（使用 React 18 原生 Hook） */
